...
int ringbuffer_write(rbctx_t *context, void *message, size_t message_len) {
  ...
  while (1) {
    ...
    int res = pthread_cond_timedwait(&context->signal_write,
                                     &context->mutex_write, &timeout);
    if (res == ETIMEDOUT) {
      pthread_mutex_unlock(&context->mutex_write);
      return RBUF_TIMEOUT;
    }
  ...
  if (context->write + sizeof(size_t) <= context->end) {
    ...
    memcpy(context->begin, (uint8_t *)&message_len + first_part_size,
           sizeof(size_t) - first_part_size);
  ...
  if (context->write + message_len <= context->end) {
    ...
    memcpy(context->begin, (uint8_t *)message + first_part_size,
           message_len - first_part_size);
...
int ringbuffer_read(rbctx_t *context, void *buffer, size_t *buffer_len) {
  ...
  while (context->read == context->write) {
    int res = pthread_cond_timedwait(&context->signal_read,
                                     &context->mutex_read, &timeout);
    if (res == ETIMEDOUT) {
