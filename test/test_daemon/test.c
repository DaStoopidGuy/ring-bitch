...
int main() {
  ...
  /* execute daemon */
  printf("Executing daemon! If it does not terminate (in 10s), it is likely "
         "stuck\n");
  printf("You may need to kill it manually (CTRL+C) and compare the files with "
         "'diff'\n");
  printf("If the files are the same, you are nearly done, and have to worry "
         "only about pthread_cancel logic!\n\n");
  ...
  if (check_files("11.txt", "test/test_daemon/rndtxt1_lsg.txt") != 0) {
    fprintf(stderr, "Error: files 11.txt and test/test_daemon/rndtxt1_lsg.txt "
                    "are not the same\n");
    return 1;
  }

  if (check_files("12.txt", "test/test_daemon/rndtxt2_lsg.txt") != 0) {
    fprintf(stderr, "Error: files 12.txt and test/test_daemon/rndtxt2_lsg.txt "
                    "are not the same\n");
    return 1;
  }

  if (check_files("13.txt", "test/test_daemon/rndtxt3_lsg.txt") != 0) {
    fprintf(stderr, "Error: files 13.txt and test/test_daemon/rndtxt3_lsg.txt "
                    "are not the same\n");
    return 1;
  }
